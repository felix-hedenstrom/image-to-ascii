...
case class Image[P <: Pixel](rows: Vector[ImageRow[P]]) {
  ...
  def scaleDimensionToFit(n: Int)(implicit monoid: Monoid[P], brightness: Brightness[P]): Image[P] = {
    ...
    Image((0 until (ratio * height).ceil.toInt).map(y =>
      ImageRow((0 until (ratio * width).ceil.toInt).map { x =>

        // average pixels
        val subpixels = (0 until (1 / ratio).toInt).flatMap(dy =>
          (0 until (1 / ratio).toInt).flatMap(dx =>
            get((x/ratio).toInt + dx, (y/ratio).toInt + dy)
          )
        )
...
object Image {
  ...
  def fromBufferedImage(bufferedImage: BufferedImage): Image[ColoredPixel] = {
    Image(
      (bufferedImage.getMinY until bufferedImage.getHeight).map(y =>
        ImageRow(
          (bufferedImage.getMinX until bufferedImage.getWidth).map(x =>
            Pixel.ColoredPixel.fromRgbInt(bufferedImage.getRGB(x, y)
            )
          ).toVector
        )
      ).toVector
